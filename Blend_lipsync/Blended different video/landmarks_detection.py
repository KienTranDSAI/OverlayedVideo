...
if __name__ == "__main__":
    # detector = create_landmarks_dectection_model()

    # # STEP 3: Load the input image.
    # # image = mp.Image.create_from_file("image.png")
    # # cv_mat = cv2.imread("image.png")
    # cv_mat = cv2.imread("myFrame.png")
    # image = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv_mat)
    # # STEP 4: Detect face landmarks from the input image.
    # # detection_result = detector.detect(image)
    # a = get_landmarks(detector, cv_mat)
    # print(len(a))
   ...
   # STEP 2: Create an FaceLandmarker object.
    ...
    # STEP 3: Load the input image.
    image = mp.Image.create_from_file("myFrame.png")

    # STEP 4: Detect face landmarks from the input image.
    detection_result = detector.detect(image)

    a = detection_result.face_landmarks
    print(len(a))
    img = cv2.imread("myFrame.png")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = mp.Image(image_format=mp.ImageFormat.SRGB, data=img)
    
    detection_result = detector.detect(img)
    b = detection_result.face_landmarks
    print(len(b))
    # img = cv2.imread("myFrame.png")
    # a = get_landmarks(detector, img)
    # # a = detection_result.face_landmarks
    # print(len(a))
    # STEP 5: Process the detection result. In this case, visualize it.
